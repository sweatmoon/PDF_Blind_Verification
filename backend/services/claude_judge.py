...
━━━ 항목 출력 원칙 (매우 중요) ━━━
- 한 페이지에 위반/주의 요소가 여러 개라면 각각 별도 items 항목으로 출력하라. 절대 하나로 묶거나 생략하지 말라.
- 같은 로고·워터마크라도 각 페이지에서 개별 항목으로 기재하라. (범위 묶기 금지)
- 업체명, 인력명, 이메일, 도메인, 로고 등 유형이 다른 위반은 반드시 별도 항목으로 분리 출력하라.
- 같은 페이지에 동일 유형 위반이 여러 건이면 모두 별도 항목으로 출력하라.

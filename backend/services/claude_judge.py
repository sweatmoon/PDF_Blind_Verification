...
【실제 인물 사진 → 위반】 — 아래 조건을 모두 충족해야 위반 판정 가능
필수 조건 (모두 충족해야 함):
  1. 피부 질감이 사진처럼 보인다 (픽셀 질감, 사진 해상도)
  2. 눈·코·입 이목구비가 실제 사람처럼 식별 가능하다
  3. 카메라로 촬영된 실사 이미지처럼 보인다
→ 위 3가지를 90% 이상 확신할 때만 【위반】
→ 하나라도 불확실하면 반드시 【허용】

해당하는 경우:
- 피부 질감이 보이는 실사 인물 사진
- 눈·코·입이 명확히 식별되는 사진
- 인물 프로필 사진, 행사/인터뷰 촬영 사진
- 명함 사진, 여러 인물의 얼굴 사진 배열

【절대 위반 불가 — 아래 유형은 무조건 【허용】】
★★★ 아래 중 하나라도 해당하면 절대 위반 판정 금지 ★★★
- 사람 아이콘 (단색, 선화, 색상 무관)
- 픽토그램 (화장실 표지판 스타일 포함)
...
def _post_process_faces(items: List[dict]) -> List[dict]:
    ...
    ★ 핵심 로직 변경 (오탐 방지):
    기존: 실루엣/아이콘 키워드 있으면 허용 (없으면 위반 유지)
    변경: 실제 사진 키워드가 명시적으로 있을 때만 위반 유지
          → 그 외 모든 경우 허용 (기본값 = 허용)

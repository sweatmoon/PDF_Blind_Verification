...
【우선순위 4 — 인물 사진 (위반)】
- 실제 사람의 얼굴·상반신이 찍힌 실사 사진 (프로필 사진, 명함 사진 포함) → 무조건 즉시 【위반】
- 인력 소개 레이아웃에 실사 사진이 있는 경우 → 【위반】
- 사진만 있고 이름이 완전히 마스킹된 경우도 실사 사진 자체는 위반
- 여러 명의 실사 사진이 격자 배치된 경우도 위반

★ 반드시 허용해야 하는 인물 이미지 (절대 위반 아님) ★
- 단색(검은색·회색·파란색 등) 실루엣 아이콘: 얼굴·피부색 없이 외형 윤곽선만 있는 경우 → 【허용】
- 픽토그램·아이콘·일러스트·캐릭터: 만화체·단순화된 그림체 → 【허용】
- 정장 실루엣, 인물 형태 아이콘 등 UI에 흔히 쓰이는 사람 모양 도형 → 【허용】
- 시스템 아키텍처·플로우차트·다이어그램 안에 포함된 사람 아이콘 → 【허용】
- 실루엣이라도 특정인을 식별할 수 없으면 무조건 【허용】
- 판단 기준: "이 이미지에서 실제 사람의 얼굴(피부색, 이목구비)이 명확히 보이는가?" → NO이면 반드시 허용
- 주의: 다이어그램·플로우차트 안의 사람 아이콘은 절대 인물사진 위반이 아님

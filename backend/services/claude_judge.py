...
- 2~4글자 한글 이름 + 직책(기술사·감리사·책임자·매니저·과장·부장 등)이 함께 있으면 → 무조건 【위반】
- 2~4글자 한글 이름 + 학력/경력 정보가 함께 있으면 → 무조건 【위반】
- 인물 사진·실루엣 옆에 이름처럼 보이는 한글 텍스트 → 무조건 【위반】
- 인력 소개 카드·프로필 박스·조직도에서 한글 이름 발견 → 무조건 【위반】
- OOO·○○○·***·홍○○ 형태로 가려진 경우만 익명 처리로 인정 → 【허용】
- 익명 처리가 됐더라도 직책·학력 조합으로 특정인 유추 가능하면 → 【주의】
...
【허용】발주기관명/로고, 실적 발주처명, 공공기관명, 사업명, 일반 기술/오픈소스명, 완전한 익명 처리, 일반 아이콘

━━━ 판정 우선순위 ━━━
1. 이름+직책 조합이 보이면 즉시 위반 — 다른 맥락과 무관하게 적용
2. 허용 목록에 해당하면 허용
3. 그 외 애매한 경우만 주의

...
포털 제안작업표 HTML의 감리일정 표에는 단계별로 다음 4개 컬럼이 존재한다:
  [예비조사(MD)] / [감리(MD)] / [조치확인(MD)] / [제안(MD)]

### 반드시 지켜야 할 규칙
1. **HTML공수(MD) = 반드시 "제안(MD)" 컬럼 값만 사용한다.**
2. "제안(MD)" = 감리원 공수 + 전문가 공수의 합계 (포털이 자동 계산한 최종값)
3. [예비조사(MD)], [감리(MD)], [조치확인(MD)] 컬럼은 절대 사용하지 않는다.
4. 감리원 행의 숫자만 따로 합산하는 것은 **오류**이다.

### 금지 패턴 (이렇게 하면 틀린다)
- ❌ 감리원 공수만 합산: 19+14+8+16+11 = 68 또는 19+20+10+20+20 = 89 등 → 오답
- ❌ [예비조사(MD)] 또는 [감리(MD)] 컬럼 값 사용 → 오답
- ❌ "합계" 행이 아닌 개별 감리원/전문가 행을 더함 → 오답

### 올바른 계산 방법
- ✅ 각 단계의 [제안(MD)] 값만 추출 → 합산
- ✅ 예시: 요구정의 19 + 설계 45 + 구현 34 + 종료 35 + 상시 15 = **합계 148MD**
- ✅ schedule 배열의 "HTML공수(MD)" 컬럼에 이 [제안(MD)] 값을 기입한다.

### 주의
포털 HTML에서 search_document로 공수를 조회할 때, 결과에 여러 숫자가 보이더라도
반드시 "제안(MD)" 레이블이 붙은 컬럼의 값을 사용하라.
"감리원", "전문가", "예비조사", "감리", "조치확인" 행/컬럼의 숫자는 무시한다.

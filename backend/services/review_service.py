...
⚠️ 이 제한은 critical/major/minor/checkNeeded에만 적용된다. typoChecklist(오탈자)는 이 제한에서 완전히 제외된다 — 발견한 오탈자는 개수와 무관하게 전부 기록한다.
...
1. critical + major + minor의 합계가 10을 초과하면, 근거가 가장 약한 항목부터 삭제하여 10 이하로 맞춘다. (typoChecklist는 이 계산에 포함하지 않는다 — 오탈자는 개수 제한 없이 전부 유지한다)

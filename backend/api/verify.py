...
# ── 업로드 & 검증 시작 ─────────────────────────────────────────
@router.post("/upload")
async def upload_and_verify(bg: BackgroundTasks,
...
<style>body{{font-family:'Malgun Gothic',sans-serif;max-width:1200px;margin:0 auto;padding:20px}}
...
<p><b>파일:</b> {r.get("filename","")} | <b>페이지:</b> {r.get("total_pages",0)} |
<b>검증일:</b> {str(r.get("created_at",""))[:19]}</p>
<p><b>블라인드 위험도:</b>
<span style="background:{rc};color:#fff;padding:3px 12px;border-radius:20px;font-weight:700">{risk}</span></p>
<div class="stats">
<div class="card"><div class="num" style="color:#ef4444">{vc}</div>위반</div>
<div class="card"><div class="num" style="color:#f59e0b">{cc}</div>주의</div>
<div class="card"><div class="num" style="color:#10b981">{ac}</div>허용</div>
</div>
<table><thead><tr><th>페이지</th><th>검출유형</th><th>검출내용</th>
<th>판정</th><th>판정 사유</th><th>수정 권고</th></tr></thead>
<tbody>{rows}</tbody></table>
<p style="margin-top:30px;font-size:11px;color:#94a3b8">본 리포트는 자동 검수 시스템에 의해 생성되었습니다.</p>
</body></html>"""
